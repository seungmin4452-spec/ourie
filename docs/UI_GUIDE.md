...
- 커플별 `theme_color`(`DATABASE.md` 참고)는 `--color-primary`를 오버라이드하는 방식으로 적용
- Tailwind 도입 시 위 토큰을 CSS 변수로 선언하고 `tailwind.config`에서 참조하는 구조 권장
...
## 6. 컴포넌트 가이드 (초안)
- **Button**: primary(강조색 채움) / secondary(아웃라인) / ghost(텍스트만) 3종
- **Card**: 기본 radius `16px`, 그림자는 은은하게 (`shadow-sm` 수준)
- **Badge**: 작성자 표시, 태그 표시용, pill 형태
- **BottomSheet**: 지도 상세, 사진 확대 등에서 재사용
...
## 9. 미결 사항
- 실제 브랜드 컬러/로고 확정 전이므로 위 팔레트는 임시값
- 커스텀 폰트 도입 여부
- 다크모드 수동 전환 지원 여부
