...
-- 어긋나지 않게 같이 반영한다.
--
-- **이 파일의 send_poke는 낡았다.** 여기서는 보낸 사람 이름을 profiles.nickname
-- 에서 읽는데, 그 컬럼은 앱 이름이었고 이후 app_name으로 rename됐다. 지금 이
-- 파일만 실행하면 함수가 "column nickname does not exist"로 죽는다.
-- 2026-08-11-names.sql을 이어서(또는 대신) 실행할 것. 여기 남겨두는 이유는
-- 이미 한 번 적용된 기록이기 때문이다.
-- ============================================================
